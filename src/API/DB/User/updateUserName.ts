...
const updateUserName = async (userEmail: string, name: string): Promise<boolean> => {
...
const _isValid = async (name: string): Promise<boolean> => {
  if (!name) {
  ...
  name = name.toLowerCase();
  if (name.length < 3) {
    HandleMessage("Имя должно быть длинее 3 символов", "error");
    return false;
  }

  if (name.length > 12) {
    HandleMessage("Имя не должно быть длинее 12 символов", "error");
    return false;
  }
